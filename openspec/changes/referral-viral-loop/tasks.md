...
- [ ] 1.5 Add index `idx_referral_redemptions_referrer` on `referral_redemptions(referrer_voter_id)`
- [ ] 1.6 Add index `idx_referral_redemptions_created` on `referral_redemptions(created_at)`

## 2. Backend: Referral stats endpoint

- [ ] 2.1 Add `ReferralStats` response struct (ref_code, referred_count, extra_vote_available)
- [ ] 2.2 Add referral code generation: `substr(sha256(voter_id || ip_salt), 0, 12)` as a method on AppState, with collision check and base36 counter suffix (00–zz, max 1296 attempts)
...
- [ ] 3.4 Add `extra_vote_available()` check in `backend/src/db.rs` (atomically insert `extra_votes` on the 3rd/6th/9th/… redemption; max 2 consumed in 24h)
...
- [ ] 7.6 Add progress indicator bar to SharePanel referral section (X/3, progress bar width)
- [ ] 7.7 Add celebration animation when extra_vote_available transitions to true (confetti effect)
